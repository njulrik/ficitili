...
class BoundedSearch :
    ...
    def find_and_insert (self, marking) :
        try :
            return self.states.nodes[marking]['org']
        except KeyError :
...
def test4 () :
    ...
    print(repr (m1))
    print(repr (g.nodes[m1]['original']))
    print(g[m3][m2])
...
def parse () :
    ...
    if args.max_states < 0 :
        args.max_states = sys.maxsize
    if args.mcc15_stop_after < 0 :
        args.mcc15_stop_after = sys.maxsize
    return args
...
def main () :
    ...
    try :
        f = open (args.net_path, 'rb')
        net.read (f, fmt='pnml')
        f.close ()
    except IOError as xxx_todo_changeme1 :
        (e, m) = xxx_todo_changeme1.args
